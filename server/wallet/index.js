    ...
    return Wallet.getByUserId(userId)
      .then((wallet) => {
        ...
        if (coin.availableBalance >= amount) {
          return BtcService.SendToAddress(address, amount, comment, toComment, altNode)
            .then((txId) => {
              console.log('BtcService.SendToAddress DONE: ', txId);
            ...
            .catch((error) => {
              console.log('BtcService.SendToAddress failed: BTC SERVICE CATCH', error);
...
function fetchBalances(coin) {
  ...
  return BtcService.listReceivedByAddress()
    .then((list) => {
      let balance = 0;
...
function cronJob() {
  return BtcService.generateBlocks(2)
    .then(() => {
      return BtcService.generateBlocks(2, true);
    })
    .then(() => {
